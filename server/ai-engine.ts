/**
 * AXIOM AI Autonomous Engine
 *
 * The central nervous system of AXIOM CRM. Runs continuously in the background,
...
const TASKS: AITask[] = [
    ...
    async run({ log }) {
      ...
      const aiResponse = await invokeLLM({
        messages: [
          {
            role: "system",
            content: `You are the AXIOM CRM self-healing AI. Analyze error patterns and recommend specific auto-corrections. 
    ...
    async run({ log }) {
      ...
      const aiResponse = await invokeLLM({
        messages: [
          {
            role: "system",
            content: `You are the AXIOM CRM lead scoring AI. Score each prospect 0-100 based on engagement stage, profile completeness, and recency. 
    ...
    async run({ log }) {
      ...
      const aiResponse = await invokeLLM({
        messages: [
          {
            role: "system",
            content: `You are the AXIOM CRM sequence optimization AI. Analyze sequence data and provide optimization recommendations.
  ...
  // ── Migration Field Mapper ────────────────────────────────────────────────
  {
    ...
    description: "Processes pending migration jobs, using AI to map source fields to AXIOM fields automatically.",
    category: "migration",
    priority: "high",
    intervalMinutes: 10,
    async run({ log }) {
