    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Parsed", value: emails.data?.filter((e: any) => e.parseStatus === "parsed").length || 0, icon: Brain, color: "text-purple-400" },
          { label: "Converted", value: emails.data?.filter((e: any) => e.convertedToLoad).length || 0, icon: Truck, color: "text-green-400" },
          { label: "Pending", value: emails.data?.filter((e: any) => e.parseStatus === "pending").length || 0, icon: Clock, color: "text-yellow-400" },
        ].map(s => (
      ...
      <div className="space-y-3">
        {emails.data?.map((email: any) => {
          let parsed: any = {};
          try { parsed = JSON.parse(String(email.parsedData || "{}")); } catch {}
          return (
            <Card key={email.id} className="border-border/50 hover:border-primary/30 transition-colors">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${email.parseStatus === "parsed" ? "bg-purple-500/10" : "bg-gray-500/10"}`}><Mail className={`w-5 h-5 ${email.parseStatus === "parsed" ? "text-purple-400" : "text-gray-400"}`} /></div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold">{email.subject || "No Subject"}</span>
                        <Badge className={email.parseStatus === "parsed" ? "bg-purple-500/20 text-purple-400" : "bg-yellow-500/20 text-yellow-400"}>{email.parseStatus}</Badge>
                        {email.convertedToLoad && <Badge className="bg-green-500/20 text-green-400"><CheckCircle className="w-3 h-3 mr-1" />Converted</Badge>}
                      </div>
                      <p className="text-sm text-muted-foreground mt-1">From: {email.senderEmail} · {new Date(Number(email.receivedAt)).toLocaleDateString()}</p>
                    </div>
                  </div>
                  {email.parseStatus === "parsed" && !email.convertedToLoad && (
                ...
                {parsed.origin && (
                  <div className="mt-3 p-3 rounded-lg bg-muted/20 grid grid-cols-4 gap-4 text-sm">
                    <div><p className="text-muted-foreground">Origin</p><p className="font-medium">{parsed.origin}</p></div>
                    <div><p className="text-muted-foreground">Destination</p><p className="font-medium">{parsed.destination}</p></div>
        ...
        {(!emails.data || emails.data.length === 0) && (
          <Card className="border-dashed"><CardContent className="p-12 text-center"><Mail className="w-12 h-12 mx-auto text-muted-foreground/30 mb-4" /><h3 className="text-lg font-medium">No inbound emails</h3><p className="text-sm text-muted-foreground mt-1">Forward load request emails to your Apex inbox for AI parsing</p></CardContent></Card>
        )}
      </div>
    </div>
  );
